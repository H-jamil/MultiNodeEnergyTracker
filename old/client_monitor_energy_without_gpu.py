...
class EnergyMonitor:
    def __init__(self, buffer_time_seconds: int = 2, client: EnergyClient = None):
        ...
        # Synchronization
        self.measurement_threads = 2  # Reduced to 2 (core and total)
        ...
        while not self.stop_event.is_set():
            ...
            with self.measurement_lock:
                if self.current_measurement is not None:
                    ...
                    # Add to buffer after total energy is set
...
if __name__ == "__main__":
    ...
    client = EnergyClient(
        master_ip="10.52.2.244",  # Change this to your master's IP
        control_port=5000,
        data_port=5001,
        node_id="node3"  # Change this for each client
    )
    client.connect()
